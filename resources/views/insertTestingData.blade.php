...
<head>
    ...
    <title>insert Testing Data</title>
...
{{-- modal ends --}}
        <fieldset>
            <legend >Insert Data In Testing Table</legend>
        <div class="mb-3">
...
op += "<td><button type='button' data-id='"+row.id+"' class='delete'>Delete</button></td>";
op += "<td><button type='button'  data-id='"+row.id+"' data-sid='"+row.sid+"' data-name='"+row.name+"' data-addr='"+row.address+"' class='updatebtn'>Update</button></td>";
op += "</tr>";
});
$("#result").html(op);
reload();
...
//insert data
            ...
            $(document).on("click", ".delete", function() {
                   ...
                   $.ajax({
                                  "method":"GET",
                                  url: "http://127.0.0.1:8000/api/deleteDataparam/" + id, // Concatenate id with the URL
      ...
      $("#btnNo").on("click", function () {
        ...
        
        {
           ...
           $("#insert").text("Update").attr("id", "update");
...
// Disable all other update buttons except for the current one
        ...
        {
            $("#update").text("Insert").attr("id", "insert");
